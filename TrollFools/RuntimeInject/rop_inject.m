...
static inline void opainject_log(const char *fmt, ...) {
    if (!fmt) return;
    va_list ap;
    va_start(ap, fmt);
...
kern_return_t createRemotePthread(task_t task, vm_address_t allImageInfoAddr, thread_act_t* remotePthreadOut)
{
	...
	__darwin_arm_thread_state64_set_lr_fptr(bootstrapThreadState.ts_64, make_sym_callable((void*)ropLoop));
...
bool sandboxFixup(task_t task, thread_act_t pthread, pid_t pid, const char* dylibPath, vm_address_t allImageInfoAddr)
{
    ...
    if (readExtensionNeeded) {
        char* extString = sandbox_extension_issue_file_to_process_by_pid(APP_SANDBOX_READ, dylibPath, 0, pid);
    ...
    if (executableExtensionNeeded) {
        char* extString = sandbox_extension_issue_file_to_process_by_pid("com.apple.sandbox.executable", dylibPath, 0, pid);
		size_t remoteExtStringSize = 0;
		vm_address_t remoteExtString = writeStringToTask(task, (const char*)extString, &remoteExtStringSize);
		if(remoteExtString)
		{
